...
evaluateKMeans = function(A,k,i){
  ...
  returnlist = list("centroids" = new_centroid, "labels" = label)
  return(returnlist)
}

calculate_SSE = function(A,centroid,label,k){
  sse = 0
  for(i in 1:k){
    for(j in 1:nrow(A)){
      if(label[j] == i){
        dist = calculate_Euclidean_Dist(A[j,],centroid[i,])
        sse = sse + (dist^2)
      }
    }
  }
  cat('SSE for k = ',k,' is - ',sse,'\n')
  return(sse)
}
...
#Execute Algorithm
Kmeans_Algorithm_with_Error = function(max_cluster,iter){
  ...
  all_error_matrix = matrix(nrow = max_cluster-1,ncol = 2)
  sse_matrix = matrix(nrow = max_cluster-1,ncol = 2)
  for(i in 2:max_cluster){
    returnlist = evaluateKMeans(breast_cancer_data_matrix,i,iter)
    centroid_data = returnlist$centroids
    centroid_label = returnlist$labels
    
    #Function call to calculate SSE
    sse = calculate_SSE(breast_cancer_data_matrix,centroid_data,centroid_label,i)
    sse_matrix[i-1,1] = i
    sse_matrix[i-1,2] = sse
