...
pca_ionospehere = prcomp(x = ionosphere_matrix)

#Scatter plot of Principle Component 1 & Principle Component 2 
pc = pca_ionospehere$rotation
plot(pc[,1],pc[,2],main="Ionosphere PCA Scatterplot", xlab="PC1", ylab="PC2", pch=19)

#Screeplot of Ionospehere Dataset after PCA
screeplot(pca_ionospehere,npcs = 33,type = "lines",main = "Screeplot of Ionosphere Dataset")

summary(pca_ionospehere)

reduced_ionosphere = pca_ionospehere$x

reduced_ionosphere = subset(reduced_ionosphere, select = c(1:17))
...
for(k in 2:5){
  ...
  for(run in 1:20){
    cat("run = ",run)
    returnList = EvaluateEMAlgorithm(reduced_ionosphere,k,threshold)
...
for(k in 2:5){
  ...
  for(run in 1:20){
    ...
    returnKmeans = evaluateKMeans(reduced_ionosphere,k,40)
