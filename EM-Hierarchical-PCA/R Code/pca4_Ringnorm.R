...
#Principle Component Analysis of Dataset
pca_ringnorm = prcomp(x = ringnorm_matrix)

#Scatter plot of Principle Component 1 & Principle Component 2 
pc = pca_ringnorm$rotation
plot(pc[,1],pc[,2],main="Ringnorm PCA Scatterplot", xlab="PC1", ylab="PC2", pch=19)

#Screeplot of Ringnorm Dataset after PCA
screeplot(pca_ringnorm,npcs = 20,type = "lines",main = "Screeplot of Ringnorm Dataset")

summary(pca_ringnorm)

reduced_ringnorm = pca_ringnorm$x

reduced_ringnorm = subset(reduced_ringnorm, select = c(1:17))
...
EvaluateEMAlgorithm = function(ringnorm_matrix, k, threshold) {
  ...
  while (t > threshold) {
    ...
    for (i in 1:k) {
      temp = cov.wt(x = ringnorm_matrix,
                    wt = weightedProbability[, i],
                    center = clusterMean_new[i, ])
...
for (k in 2:5) {
  ...
  for (run in 1:20) {
    cat("EM run = ", run)
    returnList = EvaluateEMAlgorithm(reduced_ringnorm, k, threshold)
...
for (k in 2:5) {
  ...
  for (run in 1:20) {
    cat("KM run = ", run)
    returnKmeans = evaluateKMeans(reduced_ringnorm, k, 50)
